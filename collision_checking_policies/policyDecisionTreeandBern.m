...
classdef policyDecisionTreeandBern < CollisionCheckPolicy
    ...
    methods
        function self = policyDecisionTreeandBern(decision_tree_data, path_edgeid_map, test_cost, mixing, update_posterior, switch_thresh)
        ...
        function setOutcome(self, selected_edge, outcome) %Interface function
            if (~self.switch_flag)
                ...
                if (child_node == 0 || child_data.active_prob <= self.switch_thresh)
                    self.switch_flag = true;
                end
