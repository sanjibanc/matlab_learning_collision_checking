...
classdef policyWeightedProbMaxTallyEdge < CollisionCheckPolicy
    ...
    methods
        function self = policyWeightedProbMaxTallyEdge(path_edgeid_map,  hyp_region, hyp_test, mixing, update_posterior, option)
        ...
        function edgeid = getEdgeToCheck(self) % Interface function
            ...
            self.test_bias = (1 - self.mixing)*test_count + self.mixing*0.5*ones(1, size(self.hyp_test,2));
            edgeid = prob_weighted_max_tally_test( self.selected_test_outcome, self.region_test, self.test_bias, self.option );
        end
