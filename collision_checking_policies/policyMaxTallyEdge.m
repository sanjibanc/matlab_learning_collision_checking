...
classdef policyMaxTallyEdge < CollisionCheckPolicy
    ...
    methods
        function self = policyMaxTallyEdge(path_edgeid_map,  hyp_region, hyp_test, mixing, update_posterior, option)
        ...
        function edgeid = getEdgeToCheck(self) % Interface function
            ...
            self.test_bias = (1 - self.mixing)*test_count + self.mixing*0.5*ones(1, size(self.hyp_test,2));
            [edgeid, cand_tests, tally] = max_tally_test( self.selected_test_outcome, self.region_test, self.test_bias, self.option );
            self.test_gain = zeros(1, size(self.hyp_test,2));
            self.test_gain(cand_tests) = tally;
        end
