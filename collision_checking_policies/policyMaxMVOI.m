...
classdef policyMaxMVOI < CollisionCheckPolicy
    ...
    methods
        function self = policyMaxMVOI(path_edgeid_map,  hyp_region, hyp_test, mixing, update_posterior)
        ...
        function edgeid = getEdgeToCheck(self) % Interface function
            ...
            self.test_bias = (1 - self.mixing)*test_count + self.mixing*0.5*ones(1, size(self.hyp_test,2));
            [edgeid, ~, self.test_gain] = max_mvoi( self.selected_test_outcome, self.region_test, self.test_bias);
        end
