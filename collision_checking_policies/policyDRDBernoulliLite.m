...
classdef policyDRDBernoulliLite < CollisionCheckPolicy
    ...
    methods
        function self = policyDRDBernoulliLite(path_edgeid_map, test_cost, hyp_region, hyp_test, mixing, update_posterior, option)
        ...
        function edgeid = getEdgeToCheck(self) % Interface function
            ...
            self.test_bias = (1 - self.mixing)*test_count + self.mixing*0.5*ones(1, length(self.test_cost));
            [edgeid, ~, self.test_gain] = direct_drd_bern_lite(  self.selected_test_outcome, self.region_test, self.test_bias, self.test_cost, self.option );
        end
