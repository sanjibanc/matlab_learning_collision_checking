...
function val = marginal_f_drd_bern_lite(  test, outcome, selected_test_outcome, region_test, test_bias)
...
mult = ((( test_bias(test)^outcome )*(( 1 - test_bias(test))^(~outcome))))^(sum(region_test(:, test), 1));
val = val1 - val2*mult;
